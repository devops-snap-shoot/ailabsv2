const init = () => {
    ...
    const conexion = indexedDb.open('Files', 1)
    return new Promise<any>(resolve => {
        ...
        conexion.onupgradeneeded = (e) =>{
            ...
            const coleccionObjetos = db.createObjectStore('Pdf',{
                keyPath: 'id'
            })
...
const update = (data: any, database: IDBDatabase) => {
    return new Promise<any>(resolve => {
        try {
            const trasaccion = database.transaction(['Pdf'],'readwrite')
            const coleccionObjetos = trasaccion.objectStore('Pdf')
...
const readById = (id: string, database: IDBDatabase) => {
    return new Promise(resolve => {
        const trasaccion = database.transaction(['Pdf'],'readonly')
        const coleccionObjetos = trasaccion.objectStore('Pdf')
